...
public :
    ...
    void make_new_root() {
        ...
        if (!new_path.empty()) {
            ...
            print_grid_with_path(new_path); // 경로를 포함한 그리드 출력
            stop_robot("make_new_root"); // 경로를 찾지 못했을 때 로봇 정지
        }
    }
...
// 로봇 이동 처리 함수
    void move_robot() {
        ...
        // 마지막 중간 지점부터 도착 지점까지 경로 처리
            if(waiting_for_obstacle_clear_){
                std::cout << "장애물 대기 중입니다. 로봇이 멈춰 있습니다." << std::endl;
                stop_robot("move_robot:마지막 지점 대기");
                while (waiting_for_obstacle_clear_) {
                    std::this_thread::sleep_for(0.1s); // 100ms 대기 후 상태 확인
                }
            }
        ...
        if (is_new_obstacle) {
            std::cout << "새로운 장애물입니다. 이동을 정지하고 새로운 경로를 탐색합니다." << std::endl;
            stop_robot("move_robot:마지막 new장애물");
            return; // 장애물 감지 시 함수 종료 및 새로운 경로 탐색 시작
        }
...
private :
    ...
    double previous_sensor_data_ = 0.0;  // 이전 센서 값을 저장
...
/*이동 함수*/
    ...
    void process_path(int x1, int y1, int x2, int y2) {
        ...
        // 로봇이 현재 바라보는 방향에 따라 이동할 방향을 결정
        Direction target_direction;
        if (x1 == x2) {
            // y 좌표가 다를 경우 (위, 아래)
            target_direction = (y1 > y2) ? UP : DOWN;
        } else if (y1 == y2) {
            // x 좌표가 다를 경우 (좌, 우)
            target_direction = (x1 > x2) ? LEFT : RIGHT;
        }

        // 현재 로봇의 방향과 이동할 방향을 비교
        if (robot_direction_ != target_direction) {
            // 이동할 방향으로 회전
            rotate_to_direction(target_direction);
        }

        // 방향이 맞춰졌으면 직진
        block = (x1 == x2) ? std::abs(y2 - y1) : std::abs(x2 - x1);
        go_straight(block);
    }

    // 로봇을 특정 방향으로 회전하는 함수
    void rotate_to_direction(Direction target_direction) {
        // 현재 로봇 방향에서 목표 방향으로 회전하는 로직
        while (robot_direction_ != target_direction) {
            // 회전이 필요한 경우 방향을 비교하여 좌회전 또는 우회전 수행
            if ((robot_direction_ == UP && target_direction == RIGHT) ||
                (robot_direction_ == RIGHT && target_direction == DOWN) ||
                (robot_direction_ == DOWN && target_direction == LEFT) ||
                (robot_direction_ == LEFT && target_direction == UP)) {
                right_turn(); // 우회전이 더 적합한 경우
            } else {
                left_turn(); // 좌회전이 더 적합한 경우
            }
        }
    }
...
/*콜백함수*/
    ...
    void check_obstacle() {
        if (input_completed_ && !is_new_obstacle) {
            // 이전 값과 현재 값의 평균 계산
            //double sensor_average = (previous_sensor_data_ + sensor_data_.x) / 2.0;
            //obstacle_detected_ = sensor_average < 30.0;  // 특정 거리 이하 시 장애물 감지
            ...
            if (obstacle_detected_ && !obstacle_processing_ && !waiting_for_obstacle_clear_) {
                ...
                if (is_new_obstacle) {
                    // std::cout <<"평균 값:"<<sensor_average<<
                    //         " | 이전 값:"<<previous_sensor_data_<<
                    //         " | 현재 값:"<<sensor_data_.x<<
                    //         " | 새로운 장애물 감지, 3초 대기 중..." << std::endl;
                    ...
                    obstacle_timer_ = this->create_wall_timer(
                        ...
                        [this]() {
                            ...
                            if (sensor_data_.x >= 30.0) {  // 장애물 사라진 경우
                                std::cout<< "장애물과의 거리 :" <<sensor_data_.x<< "  |  3초 내에 장애물이 사라졌습니다. 원래 경로를 따라갑니다." << std::endl;
        ...
        // 현재 센서 값을 이전 값으로 저장
        previous_sensor_data_ = sensor_data_.x;
    }
