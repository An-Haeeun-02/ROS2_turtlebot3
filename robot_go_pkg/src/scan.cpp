...
private:
    void topic_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
        float angle_min_rad = msg->angle_min;
        float angle_increment_rad = msg->angle_increment;

        int index_0 = static_cast<int>((0.0 - angle_min_rad) / angle_increment_rad);
        int index_90 = static_cast<int>((M_PI / 2 - angle_min_rad) / angle_increment_rad);
        int index_270 = static_cast<int>((3.0 * M_PI / 2 - angle_min_rad) / angle_increment_rad); // -90도를 270도로 대체

        // Convert meters to centimeters
        float distance_0_cm = msg->ranges[index_0] * 100.0;
        float distance_90_cm = msg->ranges[index_90] * 100.0;
        float distance_270_cm = msg->ranges[index_270] * 100.0; // 수정된 각도
        ...
        RCLCPP_INFO(this->get_logger(), "Distance to obstacle at 270 degrees: %.2f cm", distance_270_cm); // 수정된 각도

        auto message = geometry_msgs::msg::Vector3();
        message.x = distance_0_cm;
        message.y = distance_90_cm;
	message.z = distance_270_cm; // 수정된 각도
